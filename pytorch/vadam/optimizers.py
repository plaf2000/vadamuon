...
####################################
## PyTorch Optimizer for VadaMuon ##
####################################
...
class VadaMuon(Optimizer):
    """Implements VadaMuon algorithm.
    Arguments:
    ...
    def __init__(self, params, train_set_size, is_muon_param, lr=1e-3, betas=(0.9, 0.999), prior_prec=1.0, prec_init=1.0, num_samples=1, ns_depth=5, use_rms=False, weight_decay=0.0):
        if not 0.0 <= lr:
        ...
        self.is_muon_param = is_muon_param

        defaults = dict(lr=lr, betas=betas, prior_prec=prior_prec, prec_init=prec_init, ns_depth=ns_depth, use_rms=use_rms, weight_decay=weight_decay)
        super(VadaMuon, self).__init__(params, defaults)
    ...
    def step(self, closure):
        ...
        """
        if closure is None:
            raise RuntimeError('For now, VadaMuon only supports that the model/loss can be reevaluated inside the step function')
        ...
        for s in range(self.num_samples):
            ...
            for group in self.param_groups:
                for p in group['params']:
                    ...
                    p.data.addcdiv_(raw_noise, torch.sqrt(self.train_set_size * state['exp_avg_sq'] + group['prior_prec']))
            ...
            for group in self.param_groups:
                for p in group['params']:
                    ...
                    if p.grad.is_sparse:
                        raise RuntimeError('VadaMuon does not support sparse gradients')
...
class VOGN(Optimizer):
    ...
    def step(self, closure):
        ...
        # Update mean vector
        mu.addcdiv_(mu_grad_hat + torch.mul(mu, defaults['prior_prec']), Precision, value=-defaults['lr'])
        self.state['mu'] = mu
        # Clean memory
        del grad, ggn
        return loss
