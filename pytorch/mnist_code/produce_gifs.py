# Size of figure
figsize = (6, 4)

# Font sizes
title_size = 14
legend_size = 16
label_size = 14
tick_size = 14

##################
## Load Imports ##
##################

import os
import pickle
import numpy as np
import matplotlib.pyplot as plt

import matplotlib.animation as anim
plt.ioff()
#plt.ion()

from experiments import load_metric_history, exists_metric_history
from run_experiments import grid, model_params, train_params, optim_params, data_set
        
class AnimatedGif:
    def __init__(self, figsize=(12, 10)):
        self.fig = plt.figure(figsize=figsize)
        self.images = []
 
    def add(self, plot_list):
        self.images.append(plot_list)
 
    def save(self, filename, fps=1):
        animation = anim.ArtistAnimation(self.fig, self.images)
        animation.save(filename, writer='imagemagick', fps=fps)
        
# Folder for storing results
results_folder = "./../results/"

# Folder containing data
data_folder = "./../vadam/data"

animated_gif = AnimatedGif(figsize=figsize)

current_params = ""

for i, (hidden_sizes, mc, bs, prec) in enumerate(grid):

    if current_params != (hidden_sizes, mc, bs):
        if animated_gif.images:
            hidden_sizes_, mc_, bs_ = current_params
            print("No experiments found, skipping gif creation...")
            plt.rc('text', usetex = True)
            plt.xlim([0,train_params['num_epochs']])
            plt.ylim([0,2])
            plt.xticks(fontsize=tick_size)
            plt.yticks(fontsize=tick_size)
            plt.xlabel("Epoch", fontdict=dict(fontsize=label_size))
            plt.ylabel(r'Test $\log_{10}$loss', fontdict=dict(fontsize=label_size))
            plt.legend(["VadaMuon"],fontsize=legend_size, loc='upper center')
            plt.grid(True,which="both",color='0.75')
            plt.tight_layout()
            animated_gif.save('animations/layer' + str(len(hidden_sizes_)) + '_batchsize'+str(bs_)+'.gif')
            animated_gif = AnimatedGif(figsize=figsize)
        current_params = (hidden_sizes, mc, bs)

    model_params['hidden_sizes'] = hidden_sizes
    model_params['prior_prec'] = prec
    optim_params['prec_init'] = prec
    train_params['train_mc_samples'] = mc
    train_params['batch_size'] = bs
    if bs==1:
        train_params['num_epochs'] = 2
        evals_per_epoch = 600
    elif bs==10:
        train_params['num_epochs'] = 20
        evals_per_epoch = 60
    elif bs==100:
        train_params['num_epochs'] = 200
        evals_per_epoch = 6
    
    
    # Evaluations per epoch
    evals_per_epoch = None
    
    plot_every = 10
    
        # metrics = load_metric_history(experiment_name = "bbb_mlp_class",
        #                               data_set = data_set,
        #                               model_params = model_params,
        #                               train_params = train_params,
        #                               optim_params = optim_params,
        #                               results_folder = results_folder)
        # num_evals = len(metrics['test_pred_logloss'])
        # idx = np.arange(start = 0, stop = num_evals, step = plot_every)
        # epoch = (idx+1) * train_params['num_epochs'] / num_evals
        # met = np.array(metrics['test_pred_logloss'])
        # plot_bbb, = plt.plot(epoch, met[idx]/np.log(10), color = 'k', linestyle = '-', linewidth=2, label='BBVI')
        
        
        
        # metrics = load_metric_history(experiment_name = "vadam_mlp_class",
        #                               data_set = data_set,
        #                               model_params = model_params,
        #                               train_params = train_params,
        #                               optim_params = optim_params,
        #                               results_folder = results_folder)
        # num_evals = len(metrics['test_pred_logloss'])
        # idx = np.arange(start = 0, stop = num_evals, step = plot_every)
        # epoch = (idx+1) * train_params['num_epochs'] / num_evals
        # met = np.array(metrics['test_pred_logloss'])
        # plot_vadam, = plt.plot(epoch, met[idx]/np.log(10), color = 'r', linestyle = '-', linewidth=2, label='Vadam')

    optim_params['use_rms'] = use_rms = False
    if not exists_metric_history(experiment_name = "vadamuon_skipfirst_no_rms_mlp_class",
                                    data_set = data_set,
                                    model_params = model_params,
                                    train_params = train_params,
                                    optim_params = optim_params,
                                    results_folder = results_folder):
        print("No experiment found for", f"{hidden_sizes=}, {mc=}, {bs=}, {prec=}, skipping...")
        continue
    print("Experiment found, loading metrics...")

    metrics = load_metric_history(experiment_name = "vadamuon_skipfirst_no_rms_mlp_class",
                                    data_set = data_set,
                                    model_params = model_params,
                                    train_params = train_params,
                                    optim_params = optim_params,
                                    results_folder = results_folder)
    num_evals = len(metrics['test_pred_logloss'])
    idx = np.arange(start = 0, stop = num_evals, step = plot_every)
    epoch = (idx+1) * train_params['num_epochs'] / num_evals
    met = np.array(metrics['test_pred_logloss'])
    plot_vadamuon, = plt.plot(epoch, met[idx]/np.log(10), color = 'r', linestyle = '-', linewidth=2, label='VadaMuon')
    plot_title = plt.text(train_params['num_epochs']/2, 2.02, "Precision: " + str(prec), horizontalalignment='center', verticalalignment='bottom', fontdict=dict(fontsize=title_size))


    animated_gif.add([plot_vadamuon, plot_title])
    

    

